...
im_resp = resp["sprites"]["other"]["home"]["front_default"]
...
if len(resp["types"]) == 2:
    ...
    type_match_up_output = dict(list(type_match_up[resp_types_2['name']].items()) + list(type_match_up[resp_types_1['name']].items()))
...
temp = [x for x in type_match_up_output if type_match_up_output[x] == 1.0 or type_match_up_output[x] == 1]
for x in temp: type_match_up_output.pop(x)
for x in type_match_up_output:
    print(x + " : " + str(type_match_up_output[x]), end='')
    if list(type_match_up_output).index(x) != len(type_match_up_output)-1:
        print(", ", end="")
print()
